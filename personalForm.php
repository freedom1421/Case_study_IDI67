		...
		<select class="form-control" name="department_id" id="department_id" required="required" >
			...
			<?php
				...
				while($row=mysqli_fetch_assoc($result1)){
					...
					echo '<option value='.$row['department_id'].' ' .($department_id==$row['department_id'] ? 'selected' : '').'>'.$row['department_name'].'</option>';
				}
			?>
		</select>
	</div>
...
if ($_SESSION["userType"] == 1) {
	echo '<div class="row input-group input-group-outline">
	...
	<input type="radio" name="userType" id="userTypeadmin" value="1"';
	echo($userType== "1" ? 'checked' : '');
	echo '>
	<label for="userTypeadmin">
	: Admin (หัวหน้า)
	</label>
	<input type="radio" name="userType" id="userTypeuser" value="2"';
	echo($userType== "2" ? 'checked' : '');
	echo '>
	...
	<label class="col-form-label col-md-3 text-right" for="userType">สิทธิ์การใช้ระบบ <span class="text-danger">*</span></label>
	<div class="col-md-6 demo-radio-button">
	<input type="radio" name="userType" id="userTypeuser" value="2" checked>
	<label for="userTypeuser">
		: User (พนักงาน)
	</label>
	</div>
